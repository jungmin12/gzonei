			...
			<script>
		      function search() {
		          ...
		          if(startDate != "" || endDate != ""){
		             ...
		             document.searchForm.action = "<c:url value='/cmmnty/todayBbsList.do'/>";
...
<!--                         <li class="n2"><a>커뮤니티</a></li> -->
                        <li class="n2"><a href="<c:url value='/cmmnty/todayBbsList.do'/>" class="navi_ov">Today's 보상계획공고</a></li>                   
                    </ul>                           
                </div>
                <!-- 유틸 끝 -->   
            </div>  
												...
												<c:forEach var="list" items="${resultList}" varStatus="status">
					                              ...
					                              <c:set var="diffDate" value="${strDate - endDate}"/>
													<tr>
														...
														<td class="subject">
<%-- 															<a href="<c:url value='/cmmnty/todayBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${list.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"> --%>
															<a href="javascript:toDetail('/cmmnty/todayBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${list.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')" >
												...
												<c:forEach var="list" items="${resultList}" varStatus="status">
					                              ...
					                              <c:set var="diffDate" value="${strDate - endDate}"/>
													<tr>
														...
														<td class="subject">
															<a
															<c:if test="${sessionScope.authVO.bbs001002.infoInqireAt eq  'N'}"> onclick='viewAuthor();'</c:if>
															<c:if test="${sessionScope.authVO.bbs001002.infoInqireAt eq  'Y'}">
<%-- 															<a href="<c:url value='/cmmnty/todayBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${list.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"> --%>
															href="javascript:toDetail('/cmmnty/todayBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${list.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')"															
															</c:if>
															>
															<c:if test="${diffDate <= 3}"><span class="txt_blt02 bgc01">새글</span></c:if>
															${list.nttSj}
...
<%-- 														<td class="mo-hide"><span class="td__date"><c:out value="${fn:substring(list.frstRegisterId,0,4)}"/>***</span></td> --%>
														<td class="regDate"><span class="td__date"><fmt:formatDate pattern="yyyy-MM-dd" value="${list.frstRegistPnttm}"/></span></td>
													</tr>
													<tr class="notr"><td class="none" colspan="3"></td></tr>
											</c:forEach>
				...
				<div class="pagination-inner">
					...
					<div class="button-group">
						<a href="<c:url value='/cmmnty/todayBbsRegist.do'/>" class="btn btn-default">등록</a>
					</div>
					</c:if>
                </div>
				<!-- Paging : E -->
