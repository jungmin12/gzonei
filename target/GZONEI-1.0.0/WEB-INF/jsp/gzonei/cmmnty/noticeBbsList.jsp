			...
			<script>
		      function search() {
		          ...
		          if(startDate != "" || endDate != ""){
		             ...
		             document.searchForm.action = "<c:url value='/cmmnty/noticeBbsList.do'/>";
									...
									<c:choose>
										...
										<c:when test="${!empty resultList}">
												<c:forEach var="notice" items="${noticeList}" >
												  ...
												  <fmt:parseDate var="nDate"  value="${nowDate}" pattern="yyyy-MM-dd"/>
												  <fmt:parseNumber var="strDate" value="${nDate.time/( 24 * 60 * 60 * 1000)}" integerOnly="true" />
					                              <fmt:parseDate var="rDate"  value="${regDate}" pattern="yyyy-MM-dd"/>
					                              <fmt:parseNumber var="endDate" value="${rDate.time/( 24 * 60 * 60 * 1000)}" integerOnly="true" />
					                              <c:set var="diffDate" value="${strDate - endDate}"/>
													<tr class="notice">
														<td class="atchFileId"><span class="td__num">공지</span></td>
														<td class="subject">
															<a href="javascript:toDetail('/cmmnty/noticeBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${notice.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')">
												...
												<c:forEach var="list" items="${resultList}" varStatus="status">
					                              ...
					                              <c:set var="diffDate" value="${strDate - endDate}"/>
													<tr>
														...
														<td class="subject">
<%-- 															<a href="<c:url value='/cmmnty/noticeBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${list.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"> --%>
															<a href="javascript:toDetail('/cmmnty/noticeBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${list.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')">															
															<c:if test="${diffDate <= 3}"><span class="txt_blt02 bgc01">새글</span></c:if>
															${list.nttSj}
															</a>
														</td>
												...
												<c:forEach var="notice" items="${noticeList}" >
					                              ...
					                              <c:set var="diffDate" value="${strDate - endDate}"/>
													<tr class="notice">
														<td class="atchFileId"><span class="td__num">공지</span></td>
														<td class="subject">
<%-- 															<a href="<c:url value='/cmmnty/noticeBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${notice.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"> --%>
															<a href="javascript:toDetail('/cmmnty/noticeBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${notice.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')">
															<c:if test="${diffDate <= 3}"><span class="txt_blt02 bgc01">새글</span></c:if>
															${notice.nttSj}
															</a>
...
<!-- 														</td> -->
				...
				<div class="pagination-inner">
					...
					<div class="button-group">
						<a href="<c:url value='/cmmnty/noticeBbsRegist.do'/>" class="btn btn-default">등록</a>
					</div>
					</c:if>                
					</div>
				<!-- Paging : E -->
