...
function invtInfoInsertCk(params){
	...
	if(	params[0].value == "CM00110001"){
		if(params[5].value ==''){result ="법원을 입력바랍니다."; }
		if(params[8].value ==''){result ='사건번호를 입력바랍니다.'; }
	}
	if(	params[0].value == "CM00110002"){
		if(params[9].value ==''){result ='관리번호를 입력바랍니다.'; }
	}
	
	if(params[2].value ==''){result ='개발사업 정보를 입력바랍니다.'; }
	else if(params[3].value ==''){result ='편입 여부를 입력바랍니다.'; }
	else if(params[4].value ==''){result ='편입 면적을 입력바랍니다.'; }
	else if(params[11].value ==''){result ='주소를 시군구까지 입력바랍니다.'; }
	else if(params[14].value ==''){result ='대표 위치 입력 후, "위치입력"을 통해 경위도를 입력바랍니다.'; }
...
function invtInfoInsert(val){
	...
	if(yn){
		...
		swal(Ckresult, "필수입력값을 확인바랍니다.","warning");
	}
}
...
//	params[12].value = params[12].value.replace(/-/gi,''); 
//	params[13].value = params[13].value.replace(/-/gi,''); 
	// 개발정보에서 넘어오는 투자정보
	if(dwk_no){
		params.length = 0;
		params.push({
...
//								dataHtml += "<li class='mid_td' style='width: 95%;'>소재지 :"+area1_val+" "+area2_val+" "+data.addr+"</li>";
								...
								if(data.apasmtAmount ==0){dataHtml += "<li class='mid_td'>최저가 :"+addComma(data.bidAmount)+"원 </li>";}
								else{dataHtml += "<li class='mid_td'>최저가 :"+addComma(data.bidAmount)+"원 ( <span style='color: blue; font-weight:bold;'>"+(data.bidAmount/data.apasmtAmount*100).toFixed(2)+"%</span>)</li>";}
								if(aucYN == "경매"){
...
function invtListRef(page,num){
	...
	for(var i =(page-1)*10; i<Math.min(page*10,invtListSaveCnt); i++){
		...
		if(data.special =="Y" && checkAuthValue('ivt003004','infoInqireAt') == 'N' && data.sampleAt == null){
			if(i==num){dataHtml += '<div class="item blink " id="'+data.invtInfoSn+'" style="height:200px"><li id="tgInvt" class="primiumInv">1억 이상의 매물 (프리미엄[스폐셜]만 이용 가능)<img src="'+getContextPath()+'/images/develop/premium.png"></li></div>';}
			else{dataHtml += '<div class="item" id="'+data.invtInfoSn+'" style="height:200px"><li class="primiumInv">1억 이상의 매물 (프리미엄[스폐셜]만 이용 가능)<img src="'+getContextPath()+'/images/develop/premium.png"></li></div>';}
		}
		else{
			if(i==num){ 
...
function invtInfoDetailShow(invtInfoSn){
	...
	$.ajax({
        ...
        success : function(data){
        	...
        	document.invDetailDiv.smpleExAt.value = resData.resultList.smpleExAt;
        	document.invDetailDiv.rewardAt.value = resData.resultList.rewardAt;
        	// UUID
		...
		$.ajax({
			...
			success : function(data){
				...
				if(ts =='경매'){
					...
					document.proADetailForm.scsbidAmountAD.value = resData.resultList.scsbidAmount;
					...
					document.proPDetailForm.scsbidAmountPD.value = resData.resultList.scsbidAmount;
