...
$(document).ready(function(){
	$("[data-button='sns_share']").on("click", function(e){
		...
		if( sns_type == 'facebook' ) {
			...
			Kakao.init('effad4d17be3943669dbae719f538667');
...
Common.Dialog = {
	...
	makeModalDialog: function(data){
		...
		if(data.dialogType == 'confirm'){
			var modalFooterCancleBtn = this.createElement('button', {'type': 'button', 'class': 'btn btn-default', 'id': this.DialogInfo.cancelBtnId, 'data-dismiss': 'modal'}, modalFooterDiv);
		...
		if(data.dialogType == 'confirm' ||data.dialogType == 'alert'){
			
			$("html,body").addClass("confirm__open");
			...
			dialog.find(" #" + this.DialogInfo.okBtnId).on('click', function(){
				...
				// click enevt 제거
				$(this).off('click');
				$("html,body").removeClass("modal__open confirm__open");
			});
			dialog.find(" #" + this.DialogInfo.cancelBtnId).on('click', function(){
				...
				if(data.cancel && typeof(data.cancel) === 'function'){
					...
					console.warn('not function cancel event. dialogId: ' + data.dialogId);
				}
				// Modal 창 닫기
				dialog.modal('hide');
				// click enevt 제거
...
$(function() {
	...
	Modal.prototype.hide = function(e){
		...
		$(document.body).removeClass("modal__open confirm__open");
		$("html").removeClass("modal__open confirm__open");
...
function ajaxprototyOpen(t,m){
	...
	$.get(t.url,function(msg){
		...
		if(m.layer.length > 0){
			m.layer.html(msg);
			m.layer.find(".modal__close>button, .modal__close-button").trigger("focus");
		}else{
			m.append("<div id=\"ajaxLayer\" style=\"display:none;overflow:auto\">"+msg+"</div>");
			m.layer = $("#ajaxLayer");
			m.layer.fadeIn(300);

			var hasAllha = m.layer.find(".allHs").hasClass("allHs");
			if(hasAllha == true){
				m.layer.find(".allHs > .modal__close:nth-child(1) .modal__close-button").focus();

			}else{
				m.layer.find(".modal__close>button").focus();
			}
...
function ajaxprototypExit(t){
	...
	if ($(this).hasClass("mpopup1_close")) {
		$("#main_popup1").focus();
	}
	return true;
}
function ajaxprototypExit2(t,f){
	var t = $(t);
	var f = $(f);
	t.fadeOut(300,
		function(){
...
$(document).on('click', '[data-ly-btn]', function(){
    ...
    setTimeout(function(){
        var $popHeight = $('[data-ly-target="'+lyTarget+'"]').find('.modal__body').outerHeight();
        //console.log('deviceH : ' + $deviceHeight + '/ popupH : ' + $popHeight)
...
$(document).on('click', '[data-dismiss]', function(){
    ...
    $("html,body").removeClass("ly__open");
    $("html,body").removeClass("confirm__open");
});
