									...
									<c:otherwise>
										<c:forEach items="${resultList}" var="list" varStatus="status">
											...
											<tr>
												...
												<td class="subject">
													<a href="javascript:loginCheck('${list.nttId}','${articleNum}','${sessionScope.loginVO.gradCode}');">
...
<!-- 													<span class="td__date"> -->
<%-- 													<c:choose> --%>
<%-- 														<c:when test="${list.readngAuthorCode eq '001'}">함정사례</c:when> --%>
<%-- 														<c:when test="${list.readngAuthorCode eq '002'}">성공사례</c:when> --%>
<%-- 														<c:when test="${list.readngAuthorCode eq '003'}">실패사례</c:when> --%>
<%-- 													</c:choose>	 --%>
<!-- 													</span> -->
<!-- 												</td> -->
												<td class="mo-hide">
...
<%-- 													<c:when test="${list.readngAuthorCode eq '001'}">함정사례</c:when> --%>
<%-- 													<c:when test="${list.readngAuthorCode eq '002'}">성공사례</c:when> --%>
<%-- 													<c:when test="${list.readngAuthorCode eq '003'}">실패샘플</c:when> --%>
<%-- 												</c:choose>	 --%>
<!-- 												</span> -->
<!-- 											</td> -->
				...
				<div class="pagination-inner">
					...
					<c:if test="${sessionScope.loginVO.indvdlMberCode eq 'CM00210003'}">
						<div class="button-group">
							<a href="<c:url value='/strBbsRegist.do?readngAuthorCode=002'/>" class="btn btn-default">등록</a>
						</div>
					</c:if>
				</div>
				<!-- Paging : E -->
...
<script>
	function search() {
	    ...
	    if(startDate != "" || endDate != ""){
	       ...
	       document.searchForm.action = "<c:url value='/str/strSuccesBbsList.do'/>";
	...
	function loginCheck(nttId, articleNum, loginGrade){
		// 최근 페이징 수정 4-22
		...
		if(loginGrade == null || loginGrade == ""){
			Common.Dialog.alert({content: '로그인을 해주세요.'});
		}else{
			
			const uri = getContextPath()+ "/str/strBbsDetail.do?pageIndex="+pageIndex+"&nttId="+nttId+"&articleNum="+articleNum+"&readngAuthorCode=002&startDate="+startDate+"&endDate="+endDate+"&searchType="+searchType+"&keyword="+keyword+"&orderType="+orderType;
			const encoded = encodeURI(uri);
			console.log(encoded);
			location.href = encoded;
			
			
// 			location.href= "<c:url value='/str/strBbsDetail.do?pageIndex="+pageIndex+"&nttId="+nttId+"&articleNum="+articleNum+"&readngAuthorCode=002&startDate="+startDate+"&endDate="+endDate+"&searchType="+searchType+"&keyword="+keyword+"&orderType="+orderType+"'/>";
		}
	}
