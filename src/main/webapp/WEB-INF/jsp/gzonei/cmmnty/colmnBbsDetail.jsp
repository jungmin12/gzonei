			...
			<script>
		               ...
		               Common.Dialog.confirm({
		                    title: '삭제',
		                     ...
		                     ,ok: function(){
		                        ...
		                        ok: function(){
		                        	document.location="<c:url value='/cmmnty/colmnBbsDelete.do?nttId="+id+"'/>";
		                        }
		                     });
		                  },cancel : function(){
		                	  return false;
				...
				function pageUpdate(id){
					        Common.Dialog.confirm({
					            ...
					            ,ok: function(){
					            	document.location="<c:url value='/cmmnty/colmnBbsUpdate.do?nttId="+id+"'/>";
					            }
						        ,cancel : function(){
						        	return false;
						        }
...
<!--                                 <div class="view_cont" style="background:url('https://www.gzonei.com/file/displayFile.do?filePath=/upload/gzone/images/file/2021/05/20/BBS_9301-054210b63d49.gif&fileName=BBS_9301-054210b63d49.gif');" > -->
                                <div class="view_cont" style="background:url('../../../../images/develop/gzone_WaterMark.gif');" >
                                <!-- 게시글-->
								${result.nttCn}
								
								<h3><strong><span style="color:#e74c3c;">※이 칼럼의 내용은 정보제공의 목적으로 기술한 것이지, 세법적 또는 기타 법적 자문이나 해석을 위해 제공된 것이 아닙니다. 혹여 독자들이 기술된 내용에 따라 실행하여 발생하는 세금 또는 법적 결과에 대해 필자는 책임이 없음을 알려드립니다.</span></strong></h3>
                                <h3><strong>(주)지존 www.gzonei.com</strong></h3>
                                <!--// 게시글-->
                                </div>
                            </div>
                        </div> 
...
<%-- 									</c:choose> --%>
                    ...
                    <div class="board_btm">
                        <div class="btns_area fl_r mo_block">
                           <c:if test="${sessionScope.authVO.bbs007002.infoUpdtAt eq 'Y'}">
                            <a onclick="pageUpdate('${result.nttId}');" class="btn btn-primary">수정</a>
                            </c:if>
                           	<c:if test="${result.delAt eq 'N' && sessionScope.authVO.bbs007002.infoDelAt eq 'Y'}" >
                           	<a onclick="pageDelete('${result.nttId}');"  class="btn btn-secondary">삭제</a>
                            </c:if>
                            <c:if test="${pageIndex != null}">
<%-- 							<a href="<c:url value='/cmmnty/colmnBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"	class="btn btn-default" >목록</a> --%>
                        	<a href="javascript:toDetail('/cmmnty/colmnBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')" 	class="btn btn-default">목록</a>
                        	</c:if>
                        	<c:if test="${pageIndex == null}">
                        	<a href="<c:url value='/cmmnty/colmnBbsList.do' />"	class="btn btn-default" >목록</a>                        	
                        	</c:if>
                        </div>
                    </div>
					</div>       
