            ...
            <div id="location" class="title_wrap">
                ...
                <div class="utile_wrap">
                    ...
                    <ul class="location_wrap">
                        ...
                        <li class="n3"><a href="<c:url value='/cmmnty/focusBbsList.do'/>" class="navi_ov">지존 포커스</a></li>
                    </ul>                       
                </div>
                <!-- 유틸 끝 -->   
            </div>  
			...
			function pageDelete(id){
	               Common.Dialog.confirm({
	                    title: '삭제',
	                     ...
	                     ,ok: function(){
	                        ...
	                        ok: function(){
	                        	document.location="<c:url value='/cmmnty/focusBbsDelete.do?nttId="+id+"'/>";
	                        }
	                     });
	                  },cancel : function(){
	                	  return false;
				...
				function pageUpdate(id){
					 Common.Dialog.confirm({
				            ...
				            ,ok: function(){
				            	document.location="<c:url value='/cmmnty/focusBbsUpdate.do?nttId="+id+"'/>";
				            }
					        ,cancel : function(){
					        	return false;
					        }
...
<%-- 									</c:choose> --%>
                    ...
                    <div class="board_btm">
                         <div class="btns_area fl_r mo_block">
                           <c:if test="${sessionScope.authVO.bbs004002.infoUpdtAt eq 'Y'}">
                            <a onclick="pageUpdate('${result.nttId}');" class="btn btn-primary">수정</a>
                            </c:if>
                           	<c:if test="${result.delAt eq 'N' && sessionScope.authVO.bbs004002.infoDelAt eq 'Y'}" >
                           	<a onclick="pageDelete('${result.nttId}');"  class="btn btn-secondary">삭제</a>
                            </c:if>
                            <c:if test="${pageIndex != null}">
<%--                             <a href="<c:url value='/cmmnty/focusBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"	class="btn btn-default" >목록</a> --%>
                        	<a	class="btn btn-default" href="javascript:toDetail('/cmmnty/focusBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')">목록</a>
                        	</c:if>
                        	<c:if test="${pageIndex == null}">
                        	<a href="<c:url value='/cmmnty/focusBbsList.do' />"	class="btn btn-default" >목록</a>                        	
                        	</c:if>
                        </div>
                    </div>
					</div>       
