									...
									<c:otherwise>
										<c:forEach items="${resultList}" var="list" varStatus="status">
											...
											<tr>
												...
												<td class="subject">
													<a href="<c:url value='/adisvc/rewardPrcdntBbsDetail.do?nttId=${list.nttId}&pageIndex=${paginationInfo.currentPageNo}&articleNum=${articleNum}&listGrade=${listGrade}&seCode=instrc&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}'/>">
				...
				<div class="pagination-inner">
					...
					<c:if test="${sessionScope.loginVO.indvdlMberCode eq 'CM00210003'}">
						<div class="button-group">
							<a href="<c:url value='/adisvc/rewardPrcdntBbsRegist.do?seCode=instrc'/>" class="btn btn-default">등록</a>
						</div>
					</c:if>
				</div>
				<!-- Paging : E -->
...
<script>
	...
	function readngAuthorCheck(nttId, articleNum, listGrade){
		...
		if(isNaN(loginGrade)){
			...
			location.href= "<c:url value='/adisvc/rewardPrcdntBbsDetail.do?pageIndex="+pageIndex+"&nttId="+nttId+"&articleNum="+articleNum+"&listGrade="+listGrade+"&seCode=instrc&startDate="+startDate+"&endDate="+endDate+"&searchType="+searchType+"&keyword="+keyword+"&orderType="+orderType+"'/>";
