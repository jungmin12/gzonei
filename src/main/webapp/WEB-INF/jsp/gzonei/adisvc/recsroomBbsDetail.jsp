						...
						<c:if test="${!empty atchFileDetail}">                     
						    <div class="fileArea">
							    ...
							    <c:set var="downGrade" value="${atcFile.readingAuthorCode}"/>
							        <p>
							        <c:choose>											
										<c:when test = "${downGrade eq 'bbs009005'}"><span class="txt_blt03 state03">실버</span></c:when>
										<c:when test = "${downGrade eq 'bbs009006'}"><span class="txt_blt03 state01">골드</span></c:when>
										<c:when test = "${downGrade eq 'bbs009007'}"><span class="txt_blt03 state02">프리미엄</span></c:when>
									</c:choose>
									<c:choose>
...
<%-- 											<a onclick="downloadFile('${atcFile.orignlFileNm}','${atcFile.fileStreCours}','${downGrade}');"><c:out value="${atcFile.orignlFileNm}" /></a> --%>
											...
											<a href="javascript:toDetail('/cmmn/downloadFile.do?orignlFileNm=${atcFile.orignlFileNm}&fileStreCours=${atcFile.fileStreCours}')"><c:out value="${atcFile.orignlFileNm}" /></a>
										</c:otherwise>
									</c:choose>
...
<%--                        		<c:if test="${resultList.frstRegisterId eq loginVO.get('id') or loginVO.get('indvdlMberCode') eq 'CM00210003' }"> --%>
                       		<c:if test="${resultList.frstRegisterId eq loginVO.get('id') or sessionScope.authVO.bbs009001.infoUpdtAt eq 'Y' }">
								<a href="#" class="btn btn-primary" id="updateBtn" onclick="updateBbs(${resultList.nttId})">수정</a> 
							</c:if>
                       		<c:if test="${(resultList.frstRegisterId eq loginVO.id or sessionScope.authVO.bbs009001.infoDelAt eq 'Y') and resultList.delAt eq 'N'}">
								<a href="#"	class="btn btn-secondary" id="deleteBtn" onclick="deleteBbs(${resultList.nttId})">삭제</a>
							</c:if>
<!--                    			<a onclick="location.href = document.referrer;" class="btn btn-success" id="toList">목록</a> -->
							<c:if test="${searchVO.pageIndex != null}">	
                   			<a href="javascript:toDetail('/adisvc/recsroomBbsList.do?pageIndex=${searchVO.pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')" class="btn btn-success" id="toList">목록</a>
                        	</c:if>
                        	<c:if test="${searchVO.pageIndex == null}">
