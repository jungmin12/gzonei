	...
	$('#invDevlopContents,#nwsDevlopContents').on('click','tr',function(e){
		...
		//투자정보 입력폼에 개발사업번호 넘기기
	      if(document.invInsertDiv){
		      ...
		      document.invInsertDiv.devlopInfoSn.value = devlopInfoSn;
		      document.invDetailDiv.dwknm.value = dwknm;
		      document.invDetailDiv.dwkno.value = dwkno;
		      document.invDetailDiv.devlopInfoSn.value = devlopInfoSn;
	      }else{
		      document.insertForm.blogid.value = dwkno;
		      document.insertForm.devlopInfoSn.value = devlopInfoSn;
...
function invDevlopListSearch(val){
	...
	if(val == 2){
		$('#devArea1').val("");
		document.getElementById("devArea2").options.length=1;
		$('#devArea2').val("");
	...
	$.ajax({
        ...
        success : function(data){
        	...
        	devInfoOptionset(sortArea1List,'devArea1','전체',resData.searchVO.area1);
...
function nwsDevlopListSearch(val){
	...
	if(val == 2){
		$('#nwsArea1').val("");
		document.getElementById("nwsArea2").options.length=1;
		$('#nwsArea2').val("");
		$('#searchCondition').val("dwkNm");
		$('#pageIndexInvt').val("1");	
	...
	$.ajax({
        ...
        success : function(data){
        	...
        	devInfoOptionset(sortArea1List,'nwsArea1','전체',resData.searchVO.area1);
        	if(resData.resultListTotCnt == 0){
    			dataHtml += "<tr><td style='border: solid 1px ;text-align: center;'colspan='2'>없음</td></tr>";
    		}else{
	        	resData.resultList.forEach(function(data, index){
        	...
        	$('#nwsDevlopTotalCount').html(addComma(resData.resultListTotCnt));
