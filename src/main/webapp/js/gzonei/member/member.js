$(document).ready(function(){
	...
	$('#emplyrId').focusout(function() {
		var what = document.getElementById("emplyrId");
		var checkId = $("#emplyrId").val().replace(/\s/g, "");
		var re_id = /^[a-z0-9]{6,10}$/; //id 정규식
		var matchPart = re_id.exec(what.value);
		if(matchPart != null) {
	        return true;
	    } 
	    Common.Dialog.alert({
	        content: "아이디를 형식(영문,숫자로 6~10자)에 맞게 <br>입력해주세요."
	        ,ok : function(){
	        }
	    });
		});
	...
	$('#password').focusout(function() {
		...
		var what = document.getElementById("password");
		var matchPart = re_pw.exec(what.value);
		if(matchPart != null) {
			return true;
		} 
		Common.Dialog.alert({
			content: "비밀번호를 형식에 맞게 입력해주세요.<br>영문,숫자,특문(!@#$%^&+=)조합으로<br> 6~10자로 입력해 주세요."
				,ok : function(){
				}
		});
	});
	...
	$('#re_password').focusout(function() {
		if ($('#password').val() != $('#re_password').val()) {
			Common.Dialog.alert({
	            content: '비밀번호가 일치하지 않습니다.<br>다시 한 번 확인해주시기 바랍니다.'
	            ,ok : function(){
	            }
	        });
			return false;
