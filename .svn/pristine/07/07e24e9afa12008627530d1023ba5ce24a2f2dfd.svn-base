...
function search() {
    ...
    if(startDate != "" || endDate != ""){
       ...
       document.searchForm.action = "<c:url value='/cmmnty/qnaDevinfoList.do'/>";
        ...
        <div id="contens" class="contents_wrap">
			...
			<div class="nav-4th">
				<ul>
					...
					<li class="active"><a href="<c:url value='/cmmnty/qnaDevinfoList.do'/>">개발정보관련</a></li>
									...
									<c:choose>
										...
										<c:when test="${!empty resultList}">
												<c:forEach var="list" items="${resultList}" varStatus="status">
												  <fmt:formatDate var="nowDate"  value="${now}" pattern="yyy-MM-dd"/>
