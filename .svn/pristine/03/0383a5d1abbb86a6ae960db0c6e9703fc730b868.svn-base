    ...
    <div class="body_wrap layout">      
        <div id="contens" class="contents_wrap">
            <div id="location" class="title_wrap">
                <h2 class="page__title">Today's 보상계획공고
</h2>
                <!-- 유틸 시작 -->
			...
			function pageDelete(id){
	               Common.Dialog.confirm({
	                    title: '삭제',
	                     ...
	                     ,ok: function(){
	                        ...
	                        ok: function(){
	                        	document.location="<c:url value='/cmmnty/todayBbsDelete.do?nttId="+id+"'/>";
	                        }
	                     });
	                  },cancel : function(){
	                       return false;
				...
				function pageUpdate(id){
					 Common.Dialog.confirm({
				            ...
				            ,ok: function(){
				            	document.location="<c:url value='/cmmnty/todayBbsUpdate.do?nttId="+id+"'/>";
				            }
					        ,cancel : function(){
					        	return false; 
					        }
...
<%-- 									</c:choose> --%>
                    ...
                    <div class="board_btm">
                         <div class="btns_area fl_r mo_block">
                			<c:if test="${sessionScope.authVO.bbs001002.infoUpdtAt eq 'Y'}">
                            <a onclick="pageUpdate('${result.nttId}');" class="btn btn-primary">수정</a>
                            </c:if>
                           	<c:if test="${result.delAt eq 'N' && sessionScope.authVO.bbs001002.infoDelAt eq 'Y'}" >
                           	<a onclick="pageDelete('${result.nttId}');"  class="btn btn-secondary">삭제</a>
                            </c:if>
                            <c:if test="${pageIndex != null}">
<%--                             <a href="<c:url value='/cmmnty/todayBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"	class="btn btn-default" >목록</a> --%>
                        	<a	class="btn btn-default" href="javascript:toDetail('/cmmnty/todayBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')">목록</a>
                        	</c:if>
                        	<c:if test="${pageIndex == null}">
                        	<a href="<c:url value='/cmmnty/todayBbsList.do' />"	class="btn btn-default" >목록</a>                        	
                        	</c:if>
                        </div>
                    </div>
					</div>       
