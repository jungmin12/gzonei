...
$(document).ready(function(){
	...
	var topInfoHtml = '';
	topInfoHtml += '<b> ' + dt + ' 현재 기준</b>' +
        '개발정보 <span>' + devCount + '</span>건 / ' +
        '투자정보 <span>' + invtTotCount + '</span>건' +
        '(<span>' + invtCount + '</span>건)</p>';
...
// 	today 보상공고 
	...
	var topSlideCt = '<c:out value="${fn:length(topSlideList)}"/>';
	if(topSlideCt < 2){
		$('.slick-counter').attr("style","display:none");
		$('.slider-control').attr("style","display:none");
	}
});
...
<body id="body">
<c:if test= "${fn:length(topSlideList) > 0}">
<!-- 오픈팝업 시작 -->
<div id="op_layout">
    ...
    <div class="fix">
        ...
        <div class="layout login_util">
            <ul>
                ...
                <c:choose>
                <c:when test ="${sessionScope.loginVO.remindDay+0 eq 0}">
                	-
                </c:when>
...
<!--                 </li> -->
                ...
                <li class="box pc" style="display:none; width:35px; height:35px;margin-left:10px;margin-top: 3px;">
                    <a id="btnAllmenu" target="admin _blank" href="<c:url value='/oper/viewBbsList.do'/>" style="background:url(<%=request.getContextPath()%>/css/gzonei/SRC/images/main/system2.png) no-repeat;background-size:35px;"><span>관리자메뉴</span><i class="ir ir-tmpl"></i></a>
                </li>
                 <li class="box mo">
...
<!--                                 	<span class="yellowTxt">※</span> -->
                                	<span class="yellowTxt">우리동네 개발정보는 사업지구 Ⅰ,Ⅱ 및 준공된 사업정보만 제공</span>
                                	<div class="btnLink" onclick="window.open('about:blank').location.href='<%=request.getContextPath()%>/design/chrgeGuidanceView.do#busi_sort'"><img id="documentIcon" src="/images/develop/document.png">분류표 보기</div>
                                </div>
                            </fieldset>
                        </form>
...
<%--                     <div class="photo_list--item" style="background-image: url(<%=request.getContextPath()%>/css/gzonei/SRC/images/main/mvisual_img02.jpg)"></div> --%>
        ...
        <c:if test= "${fn:length(popSlideList) > 0}">
        <div id="popupzone">
            <div class="grap">
                <ul class="obj">                          
...
<%--             	${popSlideList.url } --%>
    ...
    <div id="mcont2">
        <div class="layout">
            <div class="img_slide" id="img_slide">
                <div class="btitle">
                    <span>NEW</span>
                    <strong>개발정보</strong>
                </div>
...
<!--                         <div class="txt_wrap" style="width:700px"> -->
                                            ...
                                            <ul>
                                            	<c:forEach items="${dsList}" var="dsData" varStatus="status">
                                            		<c:if test="${status.count % 2 ==1}"> 
                                            			<li><button type="button" class="" onclick="location.href='<%=request.getContextPath()%>/map.do?tabId=dev'">
                                            			<c:choose>
                                            			<c:when test="${fn:substring(dsData.codeId,0,4) eq 'DS01' }"><span class="yellowTxt">[Ⅰ] </span></c:when>
                                            			<c:when test="${fn:substring(dsData.codeId,0,4) eq 'DS02' }"><span class="yellowTxt">[Ⅱ] </span></c:when>
                                            			<c:when test="${fn:substring(dsData.codeId,0,4) eq 'DS03' }"><span class="redTxt">[Ⅰ] </span></c:when>
                                            			<c:when test="${fn:substring(dsData.codeId,0,4) eq 'DS04' }"><span class="redTxt">[Ⅱ] </span></c:when>
                                            			<c:otherwise></c:otherwise>
                                            			</c:choose>
                                            			 ${dsData.codeIdNm} (${dsData.devcnt})</button></li>
                                            		</c:if>
                                            	</c:forEach>
                                            </ul>
                                            <ul>	
                        ...
                        <div id="map" class="map">
                            <div class="custom_typecontrol radius_border">
						        <span id="btnRoadmap" class="kakao_btn" onclick="setMapType('roadmap')">지도</span>
						        <span id="btnSkyview" class="selected_kakao_btn" onclick="setMapType('skyview')">스카이뷰</span>
						    </div>
						    <div class="modes">
						    	<p id="viewAddr"></p>
						    </div>
