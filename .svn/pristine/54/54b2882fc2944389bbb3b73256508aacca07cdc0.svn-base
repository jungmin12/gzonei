$( document ).ready(function() {
	$('#rwdDevModal').on('click',function(e){
		devlopListSearch(2);
	});
	
	$('#rwdDevModalD').on('click',function(e){
		devlopListSearch(2);
	});
	//등록페이지 이동
	...
	//개발정보 검색 
	$('#rwdDevlopListSearch').on('click',function(e){
		devlopListSearch(1);
	});
...
function devlopListSearch(val){
	...
	if(val == 2){
		$('#rwdArea1').val("");
		document.getElementById("rwdArea2").options.length=1;
		$('#rwdArea2').val("");
	...
	$.ajax({
        ...
        success : function(data){
        	...
        	devInfoOptionset(sortArea1List,'rwdArea1','전체',resData.searchVO.area1);
