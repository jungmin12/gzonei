...
<%--                        		<c:if test="${resultList.frstRegisterId eq loginVO.get('id') or loginVO.get('indvdlMberCode') eq 'CM00210003' }"> --%>
                       		<c:if test="${resultList.frstRegisterId eq loginVO.get('id') or sessionScope.authVO.adm001002.infoUpdtAt eq 'Y' }">
								<a href="#" class="btn btn-primary" id="updateBtn" onclick="updateBbs(${resultList.nttId})">수정</a> 
							</c:if>
                       		<c:if test="${resultList.frstRegisterId eq loginVO.get('id') or sessionScope.authVO.adm001002.infoDelAt eq 'Y' }">
								<a href="#"	class="btn btn-secondary" id="deleteBtn" onclick="deleteBbs(${resultList.nttId})">삭제</a>
							</c:if>
                   			<a href="javascript:toDetail('/oper/viewBbsList.do?pageIndex=${searchVO.pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')"	class="btn btn-success" id="toList">목록</a>
                        </div>
                    </div>
...
<%-- 							<input type="hidden" id="frstRegisterId" name="frstRegisterId" value="${resultList.frstRegisterId}"> --%>
							<input type="hidden" id="frstId" name="frstId"  value="${sessionScope.loginVO.id}"/>  
							<input type="hidden" id="nttId" name="nttId" value="${resultList.nttId }">	
							<a href="#" type="submit" class="commentAddBtn submit" id="commentAddBtn">등록</a>
						</div>
				</form>
				...
				<div class="list-item">
					<c:forEach items="${commentList}" var="commentList" varStatus="status">
					    ...
					    <!-- comment 01 -->
							<div class="list-top">
						        <strong class="commentName" style="font-weight: bold;">${commentList.frstRegisterId }</strong>
							        <c:if test="${commentList.frstRegisterId eq loginVO.get('id') or loginVO.get('indvdlMberCode') eq 'CM00210003' }">
								        <div class="commentBtns" style="text-align: right; float:right;">
								        	<c:if test="${commentList.frstRegisterId eq loginVO.get('id') or loginVO.get('gradCode') eq 'CM00200014' }">
...
// 			var msg = confirm("게시글을 삭제하시겠습니까?");
			...
			Common.Dialog.confirm({
	            ...
	            ,ok: function(){
					...
					$.ajax({ 
						type : "post", 
						url : "<c:url value='/oper/deleteBbs.do'/>",
						async: false,
						dataType: "text",
						...
						success : function() { 
								...
								Common.Dialog.alert({
									...
									ok: function(){
										location.href="<c:url value='/oper/viewBbsList.do'/>"
									}
								});
							},
		...
		function updateBbs(id){
			...
			Common.Dialog.confirm({
	            ...
	            ,ok: function(){
	            	document.location="<c:url value='/oper/bbsUpdate.do?nttId="+id+"'/>";
	            }
		        ,cancel : function(){
		        	document.location.reload();
		        }
		...
		$(".commentAddBtn").on("click",function() {
			...
			var frstRegisterId = $("#frstId").val(); // 댓글작성자
		...
		function deleteComment(answerNo, nttId){
			...
			Common.Dialog.confirm({
	            ...
	            ,ok: function(){
					...
					$.ajax({ 
						type : "post", 
						url : "<c:url value='/oper/deleteComment.do'/>",
						async: false,
						dataType: "text",
				        contentType: 'application/x-www-form-urlencoded;charset=utf-8',
...
// 			var msg = confirm("댓글 수정하시겠습니까?");
			...
			Common.Dialog.confirm({
	            ...
	            ,ok: function(){
					...
					$.ajax({ 
						type : "post", 
						url : "<c:url value='/oper/updateComment.do'/>",
						async: false,
						dataType: "text",
				        contentType: 'application/x-www-form-urlencoded;charset=utf-8',
