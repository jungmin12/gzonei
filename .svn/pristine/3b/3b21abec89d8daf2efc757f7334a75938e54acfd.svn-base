...
function findUserPw() {
	if("" == document.loginForm.id.value){
		...
		$.ajax({
			type:"POST",
			url: "<c:url value='/cmmn/gzoneiSearchPassword.do'/>",
			...
			success:function(data){
				var html = '';
				if (data.result != "") {
					html = "<strong>회원님의 <b>GZONE</b> 임시비밀번호입니다.</strong><p>로그인 후 반드시 비밀번호를 변경해 주세요.</p><p>"+data.result+"</p>";
					$('#findIdBtn').text("로그인화면");
					$('#findIdBtn').attr("onClick", "javascript:moveLogin();");
				}else{   
					html = "<strong>입력하신 정보와 일치하는 회원정보가 없습니다.<br/> 다시한번 확인해주시기 바랍니다.</strong>";
				}
        ...
        <div id="contens" class="contents_wrap">
            ...
            <!-- content 영역입니다. -->
				...
				<div class="login_wrap2">
					<div class="header">
						<h4 class="modal__title"><b>GZONE</b>비밀번호가 기억나지 않으세요?</h4>
						<p>정보를 입력하시면 임시 비밀번호를 발급해드립니다.</p>
					</div>
					<div class="content">
						<form id="loginForm" name="loginForm" autocomplete="off" method="post">
				            <fieldset>  
				                ...
				                <div class="form-horizontal bucket-form">
				                	...
				                	<div class="form-group">
					                    <label for="id" class="flow">아이디</label>
						                    <input type="text" class="form-control" id="id" name="id" value="" placeholder="아이디 입력" aria-required="true">
					                </div>
				                	<div class="form-group">
					...
					<div class="footer">
						<button type="button" id="findIdBtn" class="btn btn-primary" onclick="javascript:findUserPw()">비밀번호 찾기</button>
					</div>
				</div>    
