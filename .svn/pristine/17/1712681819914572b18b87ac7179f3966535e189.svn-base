		...
		function update() {
			if(validate()){
				Common.Dialog.confirm({
							...
							ok : function() {
								document.insertForm.action = "<c:url value='/cmmnty/mmnwsBbsUdt.do'/>";
...
<div id="body_layout">	
						...
						<div class="col-sm-2 control-label col-lg-2"><div class="label">개발 지역 <span class="pilsu">필수</span></div></div>
						    ...
						    <div class="col-sm-1">
						    	<select name="nsprcCode" id="nsprcCode" class="form-control">          
						    		<option value="">-선택-</option>
						    		<option value="CM00090002" <c:if test="${result.nsprcCode eq 'CM00090002'}">selected</c:if>>디지털조선</option>
						    		<option value="CM00090003" <c:if test="${result.nsprcCode eq 'CM00090003'}">selected</c:if>>재경일보</option>
						    		<option value="CM00090004" <c:if test="${result.nsprcCode eq 'CM00090004'}">selected</c:if>>한겨레 신문</option>
						    		<option value="CM00090005" <c:if test="${result.nsprcCode eq 'CM00090005'}">selected</c:if>>조선일보</option>
						    		<option value="CM00090006" <c:if test="${result.nsprcCode eq 'CM00090006'}">selected</c:if>>중앙일보</option>
						    		<option value="CM00090007" <c:if test="${result.nsprcCode eq 'CM00090007'}">selected</c:if>>경향신문</option>
						    		<option value="CM00090008" <c:if test="${result.nsprcCode eq 'CM00090008'}">selected</c:if>>동아일보</option>
						    		<option value="CM00090009" <c:if test="${result.nsprcCode eq 'CM00090009'}">selected</c:if>>SBS TV</option>
						    		<option value="CM00090010" <c:if test="${result.nsprcCode eq 'CM00090010'}">selected</c:if>>KBS TV</option>
						    		<option value="CM00090011" <c:if test="${result.nsprcCode eq 'CM00090011'}">selected</c:if>>YBN TV</option>
									<option value="CM00090012" <c:if test="${result.nsprcCode eq 'CM00090012'}">selected</c:if>>MBC TV</option>
						    		<option value="CM00090001" <c:if test="${result.nsprcCode eq 'CM00090001'}">selected</c:if>>-직접입력-</option>
						    	</select>
						    	
							</div>
							<div class="col-sm-2">
						    	<input type="text" id="selboxDirect" name="selboxDirect" class="form-control" value="${result.nsprcNm}"/>
						    </div>
						</div> 
						<!-- 							------------관리자 권한 넣기전 조치======================= -->
							 <div class="form-group">
						    ...
						    <div class="col-sm-2 search--calendar">
							    <div>
							    	<input type="text" name="articlDe" id="articlDe" placeholder="기사일자" class="form-control" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${result.articlDe}"/>" autocomplete="off"/>
						    ...
						    <div class="col-sm-2 search--calendar">
							    <div>
							    	<input type="text" name="frstRegistPnttm" id="frstRegistPnttm" placeholder="작성일자" value="<fmt:formatDate  value="${result.frstRegistPnttm}" pattern="yyyy-MM-dd"/>" class="form-control" autocomplete="off"/>
							    	<button type="button" class="btn__calendar"><span class="skip">달력선택</span></button>
								</div>
							</div>
							</div>
	...
	<!--                               </div> -->
						...
						<div class="form-group">
						<div class="col-sm-2 control-label"><div class="label">개발 사업</div></div>
							<div class="col-sm-1">										
...
<!-- 							<div class="col-sm-2 control-label col-lg-2"><div class="label">개발 사업명</div></div>	 -->
							<div class="col-sm-4">
											<input id="dwknm" name="dwkNm"  type="text" readonly="readonly" class="form-control"  value="${result.dwkNm}"></input>
							</div>
						</div>
						<div class="form-group">
						    <div class="col-sm-12">
