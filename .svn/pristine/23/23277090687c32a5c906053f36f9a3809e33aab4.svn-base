...
<%-- 									</c:choose> --%>
                    ...
                    <div class="board_btm">
                         <div class="btns_area fl_r mo_block">
                           <c:if test="${sessionScope.authVO.bbs003002.infoUpdtAt eq 'Y'}">
                            <a onclick="pageUpdate('${result.nttId}');" class="btn btn-primary">수정</a>
                            </c:if>
                           	<c:if test="${result.delAt eq 'N' && sessionScope.authVO.bbs003002.infoDelAt eq 'Y'}" >
                           	<a onclick="pageDelete('${result.nttId}');"  class="btn btn-secondary">삭제</a>
                            </c:if>              
                            <c:if test="${pageIndex != null}">        
<%--                             <a href="<c:url value='/cmmnty/noticeBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}' />"	class="btn btn-default" >목록</a> --%>
                        	<a	class="btn btn-default" href="javascript:toDetail('/cmmnty/noticeBbsList.do?pageIndex=${pageIndex}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')">목록</a>
                        	</c:if>
                        	<c:if test="${pageIndex == null}">
                        	<a href="<c:url value='/cmmnty/noticeBbsList.do' />"	class="btn btn-default" >목록</a>                        	
                        	</c:if>
                        </div>
                    </div>
					</div>       
