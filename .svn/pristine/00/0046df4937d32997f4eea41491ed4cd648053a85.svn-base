			...
			<script>
		      ...
		      function search() {
		          ...
		          if(startDate != "" || endDate != ""){
		             ...
		             document.searchForm.action = "<c:url value='/cmmnty/mmnwsBbsList.do'/>";
				...
				function fn_egov_select_linkPage(pageNo){
					var dwkNo = "<c:out value='${dwkNo}'/>";
					var dwkNm = "<c:out value='${dwkNm}'/>";
					if( dwkNo === null || dwkNo === ""){
						...
						document.searchForm.pageIndex.value = pageNo;
						document.searchForm.action = "<c:url value='/cmmnty/mmnwsBbsList.do?dwkNm="+dwkNm+"&dwkNo="+dwkNo+"'/>";
						document.searchForm.submit();
					}
				}
...
<%--                                   <option value="nws"<c:if test="${searchVO.searchType eq 'nws' }">selected</c:if>>신문사</option> --%>
                            </select>
                        </div>
                     ...
                     <div class="sort">
                        <div class="form">
                           <select class="design-select gray-select small-select" id="orderType" name="orderType" onchange="search()" title="검색조건">
                              <option value="articl_de" <c:if test="${searchVO.orderType eq 'articl_de'}">selected</c:if>>최근 등록일순</option>
								...
								<c:when test="${sessionScope.loginVO.indvdlMberCode eq 'CM00210003'}">
									<c:choose>
										<c:when test="${empty resultList }">
											<tr>
												<td colspan="8" align="center">게시물이 없습니다.</td>
											</tr>
										</c:when>
										<c:when test="${!empty resultList}">
												...
												<c:forEach var="list" items="${resultList}" varStatus="status">
					                              ...
					                              <fmt:formatDate var="regDate"  value="${list.articlDe}" pattern="yyyy-MM-dd"/>
					                              ...
					                              <c:set var="diffDate" value="${strDate - endDate}"/>
													<tr>
														...
														<td class="nsprcNm">${list.dwkNm}<c:if test="${list.dwkNm == null || empty list.dwkNm}">부동산 뉴스</c:if></td>
														<td class="subject">
<%-- 															<a href="<c:url value='/cmmnty/mmnwsBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${fn:escapeXml(list.nttId)}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${fn:escapeXml(searchVO.keyword)}&orderType=${searchVO.orderType}' />"> --%>
															<a href="javascript:toDetail('/cmmnty/mmnwsBbsDetail.do?pageIndex=${paginationInfo.currentPageNo}&nttId=${list.nttId}&articleNum=${paginationInfo.totalRecordCount-((paginationInfo.currentPageNo-1)*paginationInfo.pageSize + status.count-1)}&startDate=${searchVO.startDate}&endDate=${searchVO.endDate}&searchType=${searchVO.searchType}&keyword=${searchVO.keyword}&orderType=${searchVO.orderType}')" >
															<c:if test="${diffDate <= 1}"><span class="txt_blt02 bgc01">새글</span></c:if>
															${list.nttSj}
															</a>
														</td>
														<td class="mo-hide">
														...
														<td class="regDate"><span class="td__date"><fmt:formatDate pattern="yyyy-MM-dd" value="${list.articlDe}"/></span></td>
...
<%-- 														<td class="regDate"><span class="td__date"><fmt:formatDate pattern="yyyy-MM-dd" value="${list.frstRegistPnttm}"/></span></td> --%>
				...
				<div class="pagination-inner">
					...
					<div class="button-group">
						<a href="<c:url value='/cmmnty/mmnwsBbsRegist.do'/>" class="btn btn-default">등록</a>
					</div>
					</c:if>
                </div>
				<!-- Paging : E -->
